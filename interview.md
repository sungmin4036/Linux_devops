...
```
개발을 하면서 기존 모놀리식 방식은 확장성, 한 물리서버에 여러 애플리케이션이 있다보니 리로스 한계 및 다른 애플리케이션에 성능저하, 많은 서버 운용으로 인한 비싼 비용이 발생

각 리소스를 나눠, 어디서 문제가 발생하고, 확장도 쉽게, 필요한 언어 자유화, 가격 낮추기, 고가용성을 위해 MSA 방식이 만들어 짐.

그러나 MSA 방식은 서비스가 너무 많이 나눠지고, 관리도 매우 복잡하여 관리가 어려워짐

이 인프라를 자동으로 관리하기 위해 컨테이너화가 필요하게 되었고, 컨테이너를 자동으로 관리해주는 K8s가 등장하게 되었다.
```

안녕하세요 데브옵스 직무에 지원한 김성민 입니다.

발표 시작하겠습니다.

저의 주요 컨셉은 비용을 최소화 하면서, 서버리스  자동화 인프라 스트럭쳐 만드는것입니다.

우선 파이프라인과 인프라 구조를 나눠서 만들었습니다. 

전체적인 도구 설정 기준은 가격, 기능, 많은 회사들의 사용, 확장성 을 기준으로 선택했습니다.

파이프 라인의  형상관리의 경우 gitlab을 사용하였습니다. 비교되는 형상관리 도구로 github과 bitbucket이 있는데,gitlab이  github 대비 많은 기능을 가지고 있고, bitbucket은 데이터 유실사건도 있고해서, 안전하고 기능많은 gitlab을 선택했습니다.

CI부분은 Jenkins로 했습니다. 조금더 빠른 처리를 위해 병렬화 작업후, 각종 테스트 및 빌드하여 이미지 생성합니다. 

이미지는 ECR에 저장하고 Argo에 보내고 배포를 합니다.

manifest 파일의 변경사항 감시 및 gitlab에 정의된 manifest 상태를 동일하게 유지하기위해 argo를 사용했습니다.

쿠버네티스의 경우 클러스터를 phase 별로 구성하는데, 그 phase 마다 배포정보가 달라서 자동관리가 필요했습니다. 그래서 helm을 추가했습니다.

Helm의 공용 chart 를 사용하여 manifest작성없이 관리가능하게 만들수 있습니다.

그리고 인증, 고가용성 등 자동으로 관리 하기위해 K8S 온프레미스로 설치하기보단, EKS를 선택했습니다. 

데이터 수집에는 프로메테우스, 데이터시각화는 그라파나 선택했습니다.

프로메테우스의 경우 scale-out 안되어 고가용성 확보가 어려운데 이를 해결하기위해서 타노스를 추가하여 고가용성을 확보하였습니다.

---

그 다음은 시스템 구조 입니다.

우선 전체적으로 서버리스 기반으로 설계를 했습니다.

서버리스 선택한 이유 설명는 서버, DB 등을 클라우드회사 에서 관리해주기 때문에 코드만 관리를 하면 함수단위로 운영이 가능하게 됩니다. 

그래서 애플리케이션을 기술에 구애받지 않게 개발이 가능하고, 높은 가용성 및 확장하는 데 필요한 모든 사항이 자동으로 처리가 가능하기 때문에 서버리스 기반으로 설계 했습니다.

고객사별 사이트 도메인을 나누기위해  route53을 넣었습니다.

Amazon Route 53는 도메인 이름을 등록 하거나 이미 소유 한 도메인 이름을 CloudFront 로 보낼 수 있습니다.

<!-- 프라이빗 호스팅 영역 기능을 사용하면 도메인 또는 하위 도메인에 대한 DNS레코드 세트를 저장하고 특정 VirtualPrivateCloud(VPC)에 대한 액세스를 제한할 수 있습니다.

특정 마이크로서비스에 대한 (IP주소,호스트 이름 및 포트) 정보를 서비스 레코드(SRV레코드)로 등록하고 관련 클라이언트 마이크로서비스의 VPC 에 대한 액세스를 제한할 수 있습니다. -->

<br>
<br>

그리고 그뒤에 cloud front를 추가했습니다.

cloudFront는 aws의 CDN 서비스로, 사용자가 요청 전송시 해당 요청이 사용자와 가까운 cloudfront pop(엣지 로케이션)으로 먼저전달 됩니다.

한번 응답한 컨텐츠를 캐시에 뒀다가, 다음번에 동일 리소스 요청시 캐시해둔 데이터로 직접 응답을줘서 사용자에게 빠르게 응답을 해주며, 오리진 서버에 부하를 줄여줍니다.

그리고, 앤드유저에 latency 를 줄여줍니다.

각 서비스를  최소화하기 위해 정적 데이터를 s3로 옮겨서 가볍게 가동 및 오토스켈링 적용 쉬워집니다.

관리형 또는 사용자 지정 인증서를 사용하여 SSL 을 활성화 할 수 있습니다.

<br>
<br>

그리고 위에있는 WAF 부분입니다. WAF는  OWASP top10 과 같은  웹 애플리케이션 보안에 대부분의 방어를 해주기 떄문에  WAF를 추가했고, 그 것에 대한 백업로그를 s3에 보관 했습니다.

이 백업로그를 원하는 형식으로 저장하기 위해 kinesis firehose를 추가했습니다.
Kinesis Firehose 를 사용하면 애플리케이션을 쓰거나 리소스를 관리할 필요가 없습니다.
Firehose는 원하는 형태로 데이터 변환후  지정한 대상으로 데이터를 자동으로 전송합니다.


그리고 사용자 관리 및 인증 과 클라우드 엑세스  관리를 위해 IAM 과 cognito을 추가했습니다.

특히 cognito의 경우 SAML2.0을 통한 엔터프라이즈 자격 증명 공급자의 로그인을 지원해서 고객사별 테넌시 분리위해 사용됩니다.(그룹기반 멀티 테넌시 사용시 cognito 이용) 또, 구글이나 페이스북과같은 소셜 자격 증명 공급자도 로그인이 가능합니다. 

이 Amazon Cognito는 서버없는 사용자 자격 증명 및 데이터 동기화 서비스로, 웹 및 모바일 앱에 사용자 가입, 로그인 및 액세스 제어를 쉽게 추가 할 수 있습니다.



APIGateway 는 트래픽 관리, 권한 부여 및 CIDR을 활용한 API에 대한 액세스 제어 등 (최대 수십만 개) 동시에 API호출 수신 및 처리와 관련된 모든 작업을 처리합니다.

(마이크로서비스를 정의하는 API를 처리하는 핵심 Component로)

선택적인 메모리 캐시에 응답을 저장하여 특정 API 요청의 성능을 향상시킬 수 있구요,

이 방법은 반복 API 요청에 대한 성능 이점을 제공 할뿐만 아니라 Lambda 함수 실행 횟수를 줄여 전체 비용을 줄일 수 있습니다.

또, API Gateway는  단일 접점 역할을 담당하여 외부로 엔드포인트 노출을 차단할수 있고, DDos 나 명령어 주입 공격으로부터 보호도 가능합니다.


서버리스 구현을위해 백엔드단은 람다로 구현했구요, 

db부분 설명드리겠습니다. Rds는 RDBMS DB 이고 Dynamodb는  nosql db 입니다.

Dynamodb가  가용성과 확장성이 좋고, 고성능이지만, 비용이 비싼 단점이 있습니다. 그중 쓰기 용량단위가 1kb 이다보니  쓰기가 가장 비쌉니다. 

그래서 쓰기 작업이 많고, 데이터 구조가 명확하고, 변경될 여지가 없을경는 데이터의 경우 RDS를 사용하고,

읽기작업이 많고, 정확한 데이터 구조를 알수없고, 데이터가 변경 및 확장되며, 업데이트가 많이 없는 데이터의 경우 dynamodb를 사용하게 했습니다.

또, RDS 분산위해 elastiCache를 사용했습니다. aws elastiCache 서비스는 쉽게 캐시 클러스터를 구성할수 있도록 제공하는 서비스 입니다.

그리고, 마스터 서버 외에 읽기전용복제본을 구성하면 부하에 더 대응가능합니다.

또 다중 에이지 기능을 이용하면 마스터 서버 장애발생시 읽기전용복제본 서버가 자동으로 마스터로 승급되어서 자동으로 읽기,쓰기 다시 지원하게 되어 시스템 고가용성 확보 가능해집니다.
