...
1. 최고의 DevOps 도구는 무엇입니까? 어떤 도구를 작업했습니까?
...
대답
. 코드로 구성은 소스 저장소에서 구성 리소스를 관리하는 것으로 설명 할 수 있습니다. 애플리케이션 구성 리소스는 버전이 지정된 아티팩트로 처리됩니다.
...
아무래도 장애 상황을 맞을 일이 있기 때문에 이에 대응하기 위해선 문제의 원인을 정확하게 파악할 필요가 있다고 생각합니다.
 ...
 또 개발팀 간의 사일로를 허물어야 하므로 사내의 다양한 구성원들과 원활하게 커뮤니케이션이 가능한 사람이 좋습니다. 무엇보다 반복적일 수 있는 인프라 관리 작업을 최대한 자동화하는 데 매력을 느끼는 사람입니다. 
 
 현재 인프라도 이전과 다르게 프로그램을 통해서 관리가 가능합니다. 이는 즉 자동화가 가능하다는 것입니다. 
 
 자동화를 통해서 휴먼 에러를 줄이고 인프라에 빠르고 안정적으로 작업을 하려고 하기 때문입니다. 더욱 효율적으로 일 하는 개발문화에 기여하고 싶은 사람이라면 꼭 지원했으면 좋겠습니다. 
